...
fn load_reference_spec() -> BTreeMap<String, HashSet<String>> {
    ...
    let doc: Value = serde_yaml::from_str(&yaml_str)
        .expect("better-auth.yaml must be valid YAML");
...
fn test_reference_spec_loads() {
    ...
    assert!(spec.len() >= 50, "Reference spec should have at least 50 paths, got {}", spec.len());
}
...
async fn test_core_endpoints_present() {
    ...
    for (method, path) in &required {
        let found = implemented.get(*path)
...
async fn test_generated_openapi_has_core_routes() {
    ...
    assert!(spec.paths.contains_key("/error"), "OpenAPI spec missing /error");
    assert!(spec.paths.contains_key("/update-user"), "OpenAPI spec missing /update-user");
    assert!(spec.paths.contains_key("/delete-user"), "OpenAPI spec missing /delete-user");
    assert!(spec.paths.contains_key("/sign-up/email"), "OpenAPI spec missing /sign-up/email");
    assert!(spec.paths.contains_key("/sign-in/email"), "OpenAPI spec missing /sign-in/email");
}
...
async fn send_json_request(auth: &BetterAuth, method: HttpMethod, path: &str, body: Option<Value>) -> (u16, Value) {
    ...
    let resp = auth.handle_request(req).await.expect("Request should not panic");
...
async fn test_contract_signup_response_shape() {
    ...
    // Must have token
    assert!(body["token"].is_string(), "Response must contain 'token' string");
    ...
    assert!(user.get("createdAt").is_some() || user.get("created_at").is_some(),
...
async fn test_contract_signout_response_shape() {
    ...
    let resp = auth.handle_request(req).await.expect("Sign-out should not panic");
...
async fn test_contract_error_response_shape() {
    ...
    assert!(status >= 400, "Error should return 4xx status, got {}", status);
    assert!(body["message"].is_string(), "Error response must have 'message' field, got: {}", body);
}
...
async fn test_contract_validation_error_shape() {
    ...
    assert!(status >= 400 && status < 500, "Validation error should be 4xx, got {}", status);
    assert!(body["message"].is_string(), "Validation error must have 'message' field");
}
...
async fn test_contract_openapi_endpoint() {
    let auth = create_full_auth().await;
    let (status, body) = send_json_request(&auth, HttpMethod::Get, "/reference/openapi.json", None).await;

    assert_eq!(status, 200);
    assert!(body["openapi"].is_string(), "Must have 'openapi' version field");
...
async fn test_contract_not_found_response() {
    let auth = create_full_auth().await;
    let (status, body) = send_json_request(&auth, HttpMethod::Get, "/nonexistent-route", None).await;

    assert_eq!(status, 404);
    assert!(body["message"].is_string(), "404 response must have 'message' field");
}
