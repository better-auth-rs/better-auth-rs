...
// Manual FromRow implementations for SQLite
...
mod sqlite_impls {
    ...
    impl FromRow<'_, SqliteRow> for Organization {
        fn from_row(row: &SqliteRow) -> Result<Self, sqlx::Error> {
            Ok(Self {
                id: row.try_get("id")?,
                name: row.try_get("name")?,
    ...
    impl FromRow<'_, SqliteRow> for Invitation {
        fn from_row(row: &SqliteRow) -> Result<Self, sqlx::Error> {
            let status_str: String = row.try_get("status")?;
            Ok(Self {
                id: row.try_get("id")?,
    ...
    impl FromRow<'_, SqliteRow> for Verification {
        fn from_row(row: &SqliteRow) -> Result<Self, sqlx::Error> {
            Ok(Self {
                id: row.try_get("id")?,
                identifier: row.try_get("identifier")?,
    ...
    impl FromRow<'_, SqliteRow> for TwoFactor {
        fn from_row(row: &SqliteRow) -> Result<Self, sqlx::Error> {
            Ok(Self {
                id: row.try_get("id")?,
                secret: row.try_get("secret")?,
    ...
    impl FromRow<'_, SqliteRow> for Passkey {
        fn from_row(row: &SqliteRow) -> Result<Self, sqlx::Error> {
            let counter_raw = row.try_get::<i64, _>("counter")?;
            let counter = u64::try_from(counter_raw).map_err(|_| {
                sqlx::Error::Decode(Box::new(std::io::Error::new(
    ...
    impl FromRow<'_, SqliteRow> for ApiKey {
        fn from_row(row: &SqliteRow) -> Result<Self, sqlx::Error> {
            let created_at: chrono::DateTime<chrono::Utc> = row.try_get("created_at")?;
            let updated_at: chrono::DateTime<chrono::Utc> = row.try_get("updated_at")?;
            let last_refill_at: Option<chrono::DateTime<chrono::Utc>> =
