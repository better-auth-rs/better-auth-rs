...
| `defer_session_refresh` | `bool` | `false` | Allow `POST /get-session`, which reads the session without refreshing it; the `POST` form answers 405 otherwise |
...
| `require_local_email_verified` | `bool` | `true` | Refuse to implicitly link a social account to an existing local account whose email is unverified |
