...
| `update_age` | `Option<Duration>` | `Some(1 day)` | Refresh session expiry when last update exceeds this duration. `None` disables refresh entirely |
| `fresh_age` | `Duration` | 1 day | Window after creation in which a session is considered "fresh" for sensitive operations |
| `cookie_name` | `String` | `"better-auth.session_token"` | Session cookie name |
...
| `cookie_cache` | `Option<CookieCacheConfig>` | `None` | Optional cookie-based session cache to skip DB lookups |

## CookieCacheConfig

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `enabled` | `bool` | `false` | Whether the cookie cache is active |
| `max_age` | `Duration` | 5 minutes | Max age before a fresh DB lookup is required |
| `strategy` | `CookieCacheStrategy` | `Compact` | Signing/encryption strategy (`Compact`, `Jwt`, or `Jwe`) |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `enabled` | `bool` | `true` | Enable request-origin CSRF checks and callback/redirect target validation |

## AdvancedConfig

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `disable_csrf_check` | `bool` | `false` | Disable request-origin and Fetch Metadata CSRF protections |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `verification_token_expiry` | `chrono::Duration` | `Duration::hours(24)` | Token validity |
