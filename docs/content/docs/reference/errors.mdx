...
`code` is present only when the upstream TypeScript implementation defines a constant for that message; the table mirrors every `defineErrorCodes` entry in `better-auth` and its plugins. Messages without an upstream code carry only `message`, so clients should branch on `code` when it exists and fall back to the status otherwise.
...
| Error | When |
|-------|------|
| `BadRequest("Challenge not found")` | Missing or invalid passkey challenge cookie |
| `BadRequest("Authentication failed")` | Invalid passkey assertion |
| `Forbidden("Passkey not found")` | Passkey credential does not exist |
