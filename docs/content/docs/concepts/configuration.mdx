...
| `update_age` | `Option<Duration>` | `Some(1 day)` |
| `fresh_age` | `Duration` | 1 day |
...
An email provider configures the global email backend used by Better Auth flows
that call `EmailProvider` directly. Configure it on the builder:
...
For password reset specifically, `POST /request-password-reset` is enabled by
`PasswordManagementPlugin::send_reset_password(...)`, not by `EmailProvider`
alone.
