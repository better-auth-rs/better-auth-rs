...
Without this setting, cookies remain scoped to the host that sets them. An empty domain string uses the hostname from `base_url`, excluding its port. This is resolved when building the auth instance; initialization fails if `base_url` is invalid or has no hostname.
