...
<Mermaid chart="sequenceDiagram\n    participant C as Client\n    participant BA as Better Auth\n    participant OP as OAuth Provider\n    C->>BA: POST /sign-in/social\n    BA-->>C: { url, redirect: true }\n    C->>OP: Redirect to provider auth URL\n    OP-->>C: User authorizes, redirect to callback\n    C->>BA: GET /callback/{provider}?code=...&state=...\n    BA->>OP: Token exchange (code + PKCE verifier)\n    OP-->>BA: Access token + Refresh token\n    BA->>OP: Fetch user info\n    OP-->>BA: User data\n    BA-->>C: { token, user }" />
...
### Existing Email
If a user with the same email already exists (from a different provider or email/password), the new provider is automatically linked to the existing user, provided the provider reports the email as verified (or is listed in `trusted_providers`) and the existing user's email is verified. The second condition is `account_linking.require_local_email_verified`, enabled by default; when either check fails the callback redirects with `error=account_not_linked`.
