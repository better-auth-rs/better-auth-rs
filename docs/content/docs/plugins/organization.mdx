...
For the bundled SeaORM store, metadata object key order is preserved only on SQLite. PostgreSQL uses `jsonb`, which does not preserve object key order, so its JSON-encoded response strings can differ from TypeScript's stored `JSON.stringify` output. Enabling `serde_json`'s `preserve_order` cannot recover order discarded by the database.
...
The bundled migrator creates nullable metadata columns for fresh databases and upgrades existing columns without changing stored values or their storage type. SQLite requires version 3.35 or newer for this upgrade and SeaORM's `RETURNING` support; the bundled SQLite version already meets this requirement.

## RBAC (Role-Based Access Control)

### Default Roles

| Role | Organization | Member | Invitation | API key |
|------|-------------|--------|------------|---------|
| **owner** | update, delete | create, update, delete | create, cancel | — |
| **admin** | update | create, update, delete | create, cancel | — |
| **member** | — | — | — | — |

The `apikey` resource governs organization-owned keys from the [API Key plugin](/docs/plugins/api-key#named-configurations-and-organization-owned-keys). Matching upstream, no default role is granted any action on it; the creator role (`creator_role`, `"owner"` by default) is allowed everything implicitly, and other roles need an explicit grant through a custom role.
...
roles.insert("editor".to_string(), RolePermissions {
    ...
    invitation: vec![],
    api_key: vec!["read".to_string(), "create".to_string()],
});
...
**Resources**: `organization`, `member`, `invitation`, `apikey`
...
| `/organization/list-user-invitations` | GET | List invitations for current user (requires a verified email) |
...
| 403 | Insufficient permissions for the operation, or `EMAIL_VERIFICATION_REQUIRED_FOR_INVITATION` when listing invitations with an unverified email |
