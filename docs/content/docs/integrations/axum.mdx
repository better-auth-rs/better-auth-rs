...
Better Auth RS provides first-class integration with [Axum](https://github.com/tokio-rs/axum) via the `axum` feature flag, including automatic route mounting, session extractors, and type-safe access to the current user.
...
Better Auth RS provides Axum extractors that automatically validate the session token and give you the current user — no manual middleware needed.
