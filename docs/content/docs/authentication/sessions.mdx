...
| `POST` | `/get-session` | Same, without refreshing the session; requires `session.defer_session_refresh`, otherwise 405 |
...
config.session.defer_session_refresh = false;
...
- `defer_session_refresh = true`: additionally accept `POST /get-session`, which returns the session without touching its expiry so a prefetching client has no side effect.
