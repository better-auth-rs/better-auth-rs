...
use better_auth::error::{AuthResult, AuthError};
...
impl DatabaseAdapter for CustomORMAdapter {
    ...
    async fn get_account(&self, _provider: &str, _provider_account_id: &str) -> AuthResult<Option<Account>> {
        Ok(None)
    }
    ...
    async fn create_verification(&self, _verification: CreateVerification) -> AuthResult<Verification> {
